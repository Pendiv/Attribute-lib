...
@Singleton
public class MaxLevelReqType extends RequirementType<Long> {
	public MaxLevelReqType() {
		super(PersistentDataType.LONG, "max_level");
	}
	...
	public Component getLoreFormat(Long value) {
		return Component.text("最大レベル: ", NamedTextColor.GREEN).append(Component.text(value));
	}

	public record MaxLevelDef(long min, long max) implements RequirementDef<MaxLevelDef> {
		public MaxLevelDef(long level) {
			this(level, level);
		}

		@Override
		public Class<? extends RequirementResolver<MaxLevelDef, EquipmentItem>> getResolverType() {
			return MaxLevelResolver.class;
		}
	}

	public static class MaxLevelResolver implements RequirementResolver<MaxLevelDef, EquipmentItem> {

		private final MaxLevelReqType type;

		@Inject
		private MaxLevelResolver(MaxLevelReqType type) {
			this.type = type;
		}
		...
		@Override
		public Collection<ItemRequirement<?>> resolver(RandomGenerator random, MaxLevelDef def, CalculationContext<? extends EquipmentItem> context) {
			return List.of(
