 ...
 *   <li>modifier 行:   {@code ns:key|sourceId|op|value|expiresAt|conditionKey}
 *       (条件なしは末尾が空。旧5フィールド形式も読める)</li>
 * </ul>
...
final class Codec {
    ...
    static String formatModifier(Modifier m) {
        return m.attribute().toString() + "|" + m.sourceId() + "|" + m.operation().name()
                + "|" + m.value() + "|" + m.expiresAt()
                + "|" + (m.condition() == null ? "" : m.condition().toString());
    }
    ...
    static Modifier parseModifier(String line) {
        ...
        if (parts.length == 6 && !parts[5].isEmpty()) {
            condition = NamespacedKey.fromString(parts[5]);
            if (condition == null) {
                return null;
            }
        }
        try {
            ...
            return new Modifier(key, parts[1], op, value, expiresAt, true, condition);
